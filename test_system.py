...
def test_system_monitor():
    ...
    for i in range(10):
        ...
        if monitor.metrics_history:
            ...
            print(f"  [{i+1}/10] CPU: {latest.cpu_percent:.1f}%, Mem: {latest.memory_percent:.1f}%")
    ...
    for i, proc in enumerate(monitor.get_top_processes(3), 1):
        print(f"  {i}. {proc.name}: {proc.cpu_percent:.1f}% CPU, {proc.memory_percent:.1f}% MEM")
...
def test_file_manager():
    ...
    try:
        ...
        print(f"  Files: {result['files_count']}, Directories: {result['directories_count']}")
        
        # Test dosyası oluştur
        test_file = "aetheros_test_file.txt"
        create_result = fm.create_file(test_file, "This is a test file created by AetherOS")
