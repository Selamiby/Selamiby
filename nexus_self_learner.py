...
class KnowledgeGraph:
    ...
    def save_graph(self):
        ...
        logger.info(
            f"Knowledge graph saved: {self.graph['statistics']['total_concepts']} concepts, "
    ...
    def add_pattern(
        self, code: str, description: str, use_cases: List[str] = None
    ) -> str:
        ...
        logger.info(
            f"Learned pattern: {pattern_id} (freq: {self.graph['code_patterns'][pattern_id]['frequency']})"
        )
        return pattern_id

    def add_relationship(
        self, from_entity: str, to_entity: str, rel_type: str, strength: float = 1.0
    ):
    ...
    def get_top_patterns(self, limit: int = 10) -> List[Dict]:
        ...
        patterns = sorted(
            self.graph["code_patterns"].items(),
...
class SelfLearner:
    ...
    def __init__(self, learning_rate: float = 5.0, aggressive: bool = True):
        ...
        logger.info(
            f"SelfLearner initialized (rate: {learning_rate}x, aggressive: {aggressive})"
        )
    ...
    def should_process_file(self, file_path: Path) -> bool:
        ...
        if (
            file_str not in self.processed_files
            or self.file_hashes.get(file_str) != current_hash
        ):
    ...
    def learn_from_workspace(self, max_files: Optional[int] = None):
        ...
        logger.info(
            f"Workspace learning complete: {learned_count} files processed in {elapsed:.1f}s"
        )
    ...
    def autonomous_learning_cycle(self, duration_seconds: Optional[int] = None):
        ...
        """
        logger.info(
            f"Starting autonomous learning cycle (duration: {duration_seconds or 'infinite'}s)"
        )
        start_time = time.time()
        cycle = 0
        ...
        while True:
            ...
            logger.info(
                f"Cycle {cycle} complete in {cycle_elapsed:.1f}s | "
    ...
    def save_stats(self):
        ...
        stats_data = {
            **self.stats,
            ...
            "last_learn_time": (
                self.stats["last_learn_time"].isoformat()
                if self.stats["last_learn_time"]
                else None
            ),
...
"""
        ...
        for i, pattern in enumerate(top_patterns, 1):
            report += (
                f"  {i}. {pattern['description'][:50]} (used {pattern['frequency']}x)\n"
            )
...
def main():
    ...
    parser.add_argument(
        ...
        type=float,
        default=5.0,
        help="Learning rate multiplier (default: 5.0)",
    )
    parser.add_argument(
        "--aggressive",
        action="store_true",
        help="Enable aggressive learning (web + more)",
    )
    parser.add_argument(
        "--duration", type=int, help="Learning duration in seconds (default: infinite)"
    )
    parser.add_argument(
        "--report", action="store_true", help="Show learning report and exit"
    )
