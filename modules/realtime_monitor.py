...
class Alert:
    ...
    def __init__(self, alert_id: str, level: AlertLevel, component: str,
                 message: str, threshold: Optional[float] = None, current_value: Optional[float] = None):
...
class RealtimeMonitor:
    ...
    def __init__(self, history_size: int = 3600):  # 1 saatlik geçmiş (60 dakika * 60 saniye)
    ...
    def _check_thresholds(self, snapshot: MetricSnapshot):
        ...
        if snapshot.cpu_percent >= self.thresholds["cpu_critical"]:
            self._create_alert(
                ...
                self.thresholds["cpu_critical"], snapshot.cpu_percent
            )
        elif snapshot.cpu_percent >= self.thresholds["cpu_warning"]:
            self._create_alert(
                "cpu_warning", AlertLevel.WARNING, "CPU",
                f"CPU usage high: {snapshot.cpu_percent:.1f}%",
                self.thresholds["cpu_warning"], snapshot.cpu_percent
            )
        ...
        if snapshot.memory_percent >= self.thresholds["memory_critical"]:
            self._create_alert(
                ...
                self.thresholds["memory_critical"], snapshot.memory_percent
            )
        elif snapshot.memory_percent >= self.thresholds["memory_warning"]:
            self._create_alert(
                "memory_warning", AlertLevel.WARNING, "Memory",
                f"Memory usage high: {snapshot.memory_percent:.1f}%",
                self.thresholds["memory_warning"], snapshot.memory_percent
            )
        ...
        if snapshot.disk_percent >= self.thresholds["disk_critical"]:
            self._create_alert(
                ...
                self.thresholds["disk_critical"], snapshot.disk_percent
            )
            ...
            self._create_alert(
                ...
                self.thresholds["disk_warning"], snapshot.disk_percent
            )
        
        # Process sayısı kontrolü
        if snapshot.process_count >= self.thresholds["process_warning"]:
            self._create_alert(
                "process_warning", AlertLevel.WARNING, "Processes",
                f"High process count: {snapshot.process_count}",
                self.thresholds["process_warning"], snapshot.process_count
            )
    ...
    def _detect_anomalies(self, snapshot: MetricSnapshot):
        ...
        if snapshot.cpu_percent > cpu_avg + (2 * cpu_std) and cpu_std > 5:
            self._create_anomaly(
                ...
                {"type": "cpu_spike", "value": snapshot.cpu_percent, "average": cpu_avg}
            )
        
        # Bellek anomalisi
        memory_values = [s.memory_percent for s in recent_snapshots]
        memory_avg = sum(memory_values) / len(memory_values)
        memory_std = (sum((x - memory_avg)**2 for x in memory_values) / len(memory_values))**0.5
        ...
        if snapshot.memory_percent > memory_avg + (2 * memory_std) and memory_std > 5:
            self._create_anomaly(
                ...
                {"type": "memory_spike", "value": snapshot.memory_percent, "average": memory_avg}
            )
        
        # Disk I/O anomalisi
        disk_io_current = (snapshot.disk_io.read_bytes + snapshot.disk_io.write_bytes) if snapshot.disk_io else 0
        
        if len(recent_snapshots) > 1:
            disk_io_prev = (recent_snapshots[-2].disk_io.read_bytes + recent_snapshots[-2].disk_io.write_bytes) if recent_snapshots[-2].disk_io else 0
    ...
    def get_current_status(self) -> Dict:
        ...
        warning_alerts = [a for a in self.alerts.values() if a.level == AlertLevel.WARNING]
    ...
    def get_alerts(self, level: Optional[AlertLevel] = None, unacknowledged_only: bool = True) -> Dict:
        """Uyarıları al"""
