...
class BackupManager:
    ...
    def schedule_backup(self, source_path: str, interval_hours: int = 24,
        ...
        def backup_thread():
            while True:
                self.create_backup(source_path, name=backup_name, 
    ...
    def list_backups(self) -> Dict:
        ...
        for name, info in self.backups.items():
            ...
            backups_list.append({
                ...
                "size": self._get_dir_size(backup_path) if backup_path.exists() else 0,
                "status": "OK" if backup_path.exists() else "MISSING"
            })
        
        return {
            "total_backups": len(backups_list),
            "backups": sorted(backups_list, 
