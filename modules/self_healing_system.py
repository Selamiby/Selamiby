...
class HealthIssue:
    ...
    def __init__(self, issue_id: str, component: str, severity: str, 
                 description: str, auto_fix: bool = False):
...
class SelfHealingSystem:
    ...
    def detect_issues(self) -> Dict:
        ...
        if memory.percent > 85:
            ...
            issue = HealthIssue(
                issue_id, "memory", "critical" if memory.percent > 95 else "high",
                f"Memory usage: {memory.percent}%", auto_fix=True
            )
        ...
        for partition in psutil.disk_partitions():
            try:
                ...
                if usage.percent > 90:
                    issue_id = f"disk_{partition.device}_{int(datetime.now().timestamp())}"
                    issue = HealthIssue(
                        issue_id, "disk", "critical" if usage.percent > 98 else "high",
                        f"Disk {partition.device}: {usage.percent}% full", auto_fix=True
                    )
                    self.issues[issue_id] = issue
        ...
        for proc in psutil.process_iter(['pid', 'name', 'memory_percent']):
            try:
                if proc.info['memory_percent'] and proc.info['memory_percent'] > 30:
                    issue_id = f"proc_{proc.info['pid']}_{int(datetime.now().timestamp())}"
                    issue = HealthIssue(
                        issue_id, "process", "high",
    ...
    def _repair_issue(self, issue: HealthIssue) -> Dict:
        ...
        if not strategy:
            return {"success": False, "error": f"No repair strategy for {issue.component}"}
    ...
    def get_health_summary(self) -> Dict:
        """Sağlık özeti al"""
        critical_count = sum(1 for i in self.issues.values() if i.severity == "critical")
