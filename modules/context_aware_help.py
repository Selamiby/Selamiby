...
class UserContextManager:
    ...
    def _find_common_files(self, actions: List) -> List[str]:
        ...
        return [
            f for f, _ in sorted(files.items(), key=lambda x: x[1], reverse=True)[:10]
        ]
    ...
    def _get_preferred_operations(self, actions: List) -> Dict:
        ...
        return dict(sorted(operations.items(), key=lambda x: x[1], reverse=True))
...
class ErrorPreventionSystem:
    ...
    def detect_risky_operation(self, operation: str, context: Dict) -> Dict:
        ...
        return {"operation": operation, "risky": len(risks) > 0, "risks": risks}
    ...
    def _is_critical_file(self, path: str) -> bool:
        """Kritik dosya mı"""
        critical_paths = ["system32", "windows", "drivers", "/etc", "/sys", "/boot"]
