...
class SystemMonitor:
    ...
    def get_metrics_summary(self, hours: int = 1) -> Dict:
        ...
        return {
            ...
            "averages": {
                "cpu_percent": round(sum(m.cpu_percent for m in recent_metrics) / len(recent_metrics), 2),
                "memory_percent": round(sum(m.memory_percent for m in recent_metrics) / len(recent_metrics), 2),
                "disk_usage_percent": round(sum(m.disk_usage_percent for m in recent_metrics) / len(recent_metrics), 2)
            },
            "maximums": {
                "cpu_percent": round(max(m.cpu_percent for m in recent_metrics), 2),
                "memory_percent": round(max(m.memory_percent for m in recent_metrics), 2),
                "disk_usage_percent": round(max(m.disk_usage_percent for m in recent_metrics), 2)
            },
    ...
    def to_dict(self) -> Dict:
        ...
        return {
            ...
            "current_metrics": asdict(self.collect_metrics()) if self.metrics_history else {},
...
if __name__ == "__main__":
    ...
    print(f"System: {monitor.system_info['platform']} {monitor.system_info['platform_release']}")
    ...
    for disk in monitor.get_disk_info():
        print(f"  {disk['mountpoint']}: {disk['used_gb']:.1f}/{disk['total_gb']:.1f} GB ({disk['percent']:.1f}%)")
