...
def test_comprehensive_matching():
    ...
    for test_error in test_cases:
        ...
        for pattern in real_patterns:
            ...
            lev_score = (
                1 - (levenshtein(error_lower, pattern_lower) / max_len)
                if max_len > 0
                else 0
            )
            ...
            token_score = (
                len(tokens1 & tokens2) / max(len(tokens1), len(tokens2))
            ...
            semantic_score = (
                1.0 if has_synonym_match(error_lower, pattern_lower) else 0.0
            )
            ...
            matching_terms = sum(
                1 for term in key_terms if term in error_lower and term in pattern_lower
            )
    ...
    print(
        f"  High Confidence: {len(high_confidence)} ({len(high_confidence)/len(results)*100:.1f}%)"
    )
    ...
    if avg_score >= 65:
        print(
            "✅ SUCCESS: Average accuracy {:.1f}% - PRODUCTION READY!".format(avg_score)
        )
    else:
