...
class GameDevMasterLearner:
    ...
    def __init__(self):
        ...
        self.learning_modules = {
            "game_engines": {
                ...
                "Unreal Engine 5": ["Blueprints", "C++", "Niagara VFX", "Nanite", "Lumen"],
                "Godot": ["GDScript", "2D/3D scenes", "Signals", "Shaders"]
            },
            "asset_creation": {
                "3D Modeling": ["Blender basics", "Mesh topology", "UV unwrapping", "LODs"],
                "Texturing": ["PBR textures", "Substance Painter", "Normal maps", "Material creation"],
                "Rigging": ["Skeleton setup", "Weight painting", "IK constraints"],
                "Animation": ["Keyframe animation", "Animation blending", "State machines"]
            },
            "npc_ai": {
            ...
            "graphics": {
                "Shaders": ["Vertex shaders", "Fragment shaders", "HLSL/GLSL", "Shader Graph"],
                "Lighting": ["Real-time lighting", "Baked lighting", "Global illumination"],
            ...
            "mobile_game": {
                "Optimization": ["Texture compression", "Mesh optimization", "Battery usage"],
            ...
            "pc_game": {
                "Graphics": ["DirectX 12", "Vulkan", "Ray tracing", "High-res textures"],
    ...
    def learn_asset_creation(self):
        ...
        for category, topics in self.learning_modules["asset_creation"].items():
            ...
            self._save_knowledge(f"asset_{category.lower().replace(' ', '_')}", knowledge)
    ...
    def learn_npc_ai(self):
        ...
        for system, topics in self.learning_modules["npc_ai"].items():
            ...
            self._save_knowledge(f"npc_ai_{system.lower().replace(' ', '_')}", knowledge)
    ...
    def learn_graphics(self):
        ...
        for category, topics in self.learning_modules["graphics"].items():
            ...
            self._save_knowledge(f"graphics_{category.lower().replace(' ', '_')}", knowledge)
    ...
    def learn_audio(self):
        ...
        for category, topics in self.learning_modules["audio"].items():
            ...
            self._save_knowledge(f"audio_{category.lower().replace(' ', '_')}", knowledge)
    ...
    def learn_mobile_game(self):
        ...
        for category, topics in self.learning_modules["mobile_game"].items():
            ...
            self._save_knowledge(f"mobile_{category.lower().replace(' ', '_')}", knowledge)
    ...
    def learn_pc_game(self):
        ...
        for category, topics in self.learning_modules["pc_game"].items():
            ...
            self._save_knowledge(f"pc_game_{category.lower().replace(' ', '_')}", knowledge)
    ...
    def generate_summary(self):
        ...
        summary = {
            ...
            "capabilities": {
                "game_engines": ["Unity (C#)", "Unreal Engine 5 (C++/BP)", "Godot (GDScript)"],
                "asset_pipeline": ["3D Modeling", "Texturing (PBR)", "Rigging", "Animation"],
                "npc_ai": ["Behavior Trees", "Pathfinding (A*)", "Decision Making", "Perception"],
                "graphics": ["Shaders (HLSL/GLSL)", "Lighting (GI)", "PBR Rendering", "Optimization"],
                "audio": ["Sound Design", "Adaptive Music", "FMOD/Wwise", "Spatial Audio"],
                "mobile_game": ["2GB optimization", "Touch controls", "Android/iOS", "Monetization"],
                "pc_game": ["15GB content", "DirectX/Vulkan", "Ray tracing", "Steam integration"]
            },
            "ready_for": {
        ...
        logger.info(f"✅ {summary['knowledge_files_created']} bilgi dosyası oluşturuldu")
