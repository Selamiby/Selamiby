        ...
        self.tasks = [
            {
                "time": "02:00",
                "action": "autogpt",
                "task": "Günün teknoloji haberlerini özetle ve raporla",
            },
            {
                "time": "03:00",
                "action": "gpt_engineer",
                "task": "Sistemin genel durumunu kontrol eden bir Python scripti yaz",
            },
            {
                "time": "04:00",
                "action": "crewai",
                "task": "Yapay zeka alanındaki yeni trendleri araştır ve bir sunum hazırla",
            },
            {
                "time": "05:00",
                "action": "stable_diffusion",
                "task": "Geleceğin teknolojisi temalı bir konsept sanat oluştur",
            },
        ]
    ...
    def run_scheduled_tasks(self):
        ...
        for task in self.tasks:
            if task["time"] == current_time:
                print(
                    f"⏰ Zamanı geldi! Görev başlatılıyor: {task['action']} - {task['task']}"
                )
    ...
    def execute_task(self, task_info):
        ...
        try:
            if action == "autogpt" and "autogpt" in self.nexus_core.modules:
                ...
                action == "stable_diffusion"
                and "stable_diffusion" in self.nexus_core.modules
            ):
