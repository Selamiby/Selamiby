...
class NEXUSAutoHealer:
    ...
    def check_yaml_files(self) -> List[Tuple[str, str, str]]:
        ...
        errors = []
        yaml_files = list(self.workspace_root.rglob("*.yml")) + list(
            self.workspace_root.rglob("*.yaml")
        )
        ...
        for yaml_file in yaml_files:
            try:
                ...
                if "secrets.VERCEL_TOKEN" in content and "VERCEL_TOKEN" not in str(
                    yaml_file
                ):
                ...
                if "secrets.DEPLOY_KEY" in content:
                    ...
                    if re.search(pattern, content):
                        errors.append(
                            (
                                str(yaml_file),
                                "Context access might be invalid: DEPLOY_KEY",
                                "YAML",
                            )
                        )
                        self.patterns["yaml_context_warnings"].append(
    ...
    def auto_fix_all(self) -> bool:
        ...
        if "powershell" in errors:
            self.log(
                f"PowerShell hataları bulundu: {len(errors['powershell'])}", "WARNING"
            )
    ...
    def report(self):
        ...
        self.log(
            f"Öğrenilmiş YAML patterns: {len(self.patterns['yaml_context_warnings'])}",
            "INFO",
        )
        self.log(
            f"Öğrenilmiş PowerShell patterns: {len(self.patterns['powershell_unused_vars'])}",
            "INFO",
        )
        ...
        try:
            ...
            subprocess.run(
                [
                    ...
                    "commit",
                    "-m",
                    "fix: NEXUS-ONE Otomatik Hata Düzeltici - Tüm sorunlar çözüldü",
                ],
                cwd=workspace,
                capture_output=True,
            )
            subprocess.run(
                ["git", "push", "origin", "main"], cwd=workspace, capture_output=True
            )
