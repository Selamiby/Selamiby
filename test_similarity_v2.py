...
def advanced_similarity(text1, text2):
    ...
    lev_score = (
        1 - (levenshtein(text1_lower, text2_lower) / max_len) if max_len > 0 else 0
    )
    ...
    matching_terms = sum(
        1 for term in key_terms if term in text1_lower and term in text2_lower
    )
...
def run_comprehensive_test():
    ...
    print(
        f"  High Confidence: {len(high_conf)} ({len(high_conf)/len(results)*100:.1f}%)"
    )
    print(
        f"  Medium Confidence: {len(medium_conf)} ({len(medium_conf)/len(results)*100:.1f}%)"
    )
