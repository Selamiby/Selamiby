...
try:
    while True:
        ...
        print("\n📊 SİSTEM DURUMU:")
        sistem_verisi = monitor.kontrol_et() if hasattr(monitor, 'kontrol_et') else {"cpu": 50, "ram": 50}
        rapor = monitor.calis() if hasattr(monitor, 'calis') else "Monitor aktif"
        print(rapor)
        
        # 2. Beyin karar versin
        print("\n🤖 BEYİN KARARI:")
        karar = beyin.karar_ver(sistem_verisi) if hasattr(beyin, 'karar_ver') else {"karar": "Devam et"}
        karar_sonucu = beyin.calistir(karar) if hasattr(beyin, 'calistir') else "Karar uygulandı"
