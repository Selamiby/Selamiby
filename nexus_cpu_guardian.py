...
class CPUGuardian:
    ...
    def should_throttle(self) -> bool:
        ...
        if stats["cpu_percent"] > self.cpu_threshold:
            logger.warning(
                f"⚠️ CPU YÜKSEK: {stats['cpu_percent']:.1f}% - THROTTLING BAŞLATIYOR"
            )
            return True

        if stats["ram_percent"] > self.ram_threshold:
            logger.warning(
                f"⚠️ RAM YÜKSEK: {stats['ram_percent']:.1f}% - THROTTLING BAŞLATIYOR"
            )
            return True
    ...
    def apply_throttle(self):
        """Pause resource-heavy operations and lower process priority."""
        logger.info("🔻 CPU Guardian: İşlemler yavaşlatılıyor...")
        self.throttled = True

        try:
            # Find top 3 CPU consuming processes that are part of the current user's session
            current_user = psutil.Process().username()
            processes = []
            for p in psutil.process_iter(["pid", "name", "cpu_percent", "username"]):
                if (
                    p.info["username"] == current_user
                    and p.info["pid"] != psutil.Process().pid
                ):
                    try:
                        # cpu_percent is a method, call it to get value
                        p.cpu_percent(interval=0.1)  # Call it once to start measurement
                        processes.append(p)
                    except (psutil.NoSuchProcess, psutil.AccessDenied):
                        continue

            time.sleep(1)  # Wait a bit for cpu_percent to be calculated
            ...
            for p in processes[:3]:
                try:
                    ...
                    logger.warning(
                        f"⬇️ Yüksek CPU'lu işlem ({p.name()}, PID: {p.pid}) önceliği düşürüldü. Yeni öncelik: {p.nice()}"
                    )
                except (psutil.NoSuchProcess, psutil.AccessDenied) as e:
                    logger.error(f"İşlem önceliği düşürülemedi (PID: {p.pid}): {e}")
    ...
    def monitor_loop(self):
        ...
        logger.info(
            f"🛡️ CPU Guardian başlatıldı (CPU: {self.cpu_threshold}%, RAM: {self.ram_threshold}%)"
        )
        ...
        while self.running:
            try:
                ...
                logger.info(
                    f"CPU: {stats['cpu_percent']:.1f}% | RAM: {stats['ram_percent']:.1f}% | Processes: {len(self.active_processes)}"
                )
