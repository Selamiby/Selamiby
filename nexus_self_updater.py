...
class SelfUpdater:
    ...
    def update_control_panel_commands(self, new_commands: List[Dict]) -> bool:
        ...
        # Insert new code
        updated_method = method_content[:inject_pos] + injection_code + method_content[inject_pos:]
        ...
        if not self.dry_run:
            ...
            logger.info(f"✅ Updated control panel with {len(new_commands)} new commands")
            self.update_log.append(f"Added {len(new_commands)} commands to control panel")
        else:
    ...
    def integrate_code_patterns(self) -> bool:
        ...
        top_patterns = sorted(patterns.items(), key=lambda x: x[1].get("frequency", 0), reverse=True)[:10]
        ...
        for pattern_id, pattern_data in top_patterns:
            logger.info(f"  - {pattern_data.get('description', 'N/A')} (freq: {pattern_data.get('frequency')})")
...
"""
    ...
    def self_update_cycle(self) -> Dict:
        ...
        logger.info(f"Self-update cycle complete: {len(results['changes'])} changes made")
        return results
...
def main():
    ...
    parser.add_argument("--dry-run", action="store_true", help="Simulate updates without applying")
    parser.add_argument("--force", action="store_true", help="Force update even with low-usage commands")
