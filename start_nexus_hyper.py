...
def start_all():
    ...
    if os.path.exists("requirements_hyper.txt"):
        run_command(
            [sys.executable, "-m", "pip", "install", "-r", "requirements_hyper.txt"],
    ...
    for script in SCRIPTS_TO_RUN:
        ...
        process = subprocess.Popen(
            [sys.executable, script], stdout=log_file, stderr=log_file
        )
    ...
    for p in psutil.process_iter(["pid", "name", "cmdline"]):
        if p.info["name"] == os.path.basename(sys.executable) and any(
            "nexus" in " ".join(p.info["cmdline"])
            or "hyper" in " ".join(p.info["cmdline"])
        ):
